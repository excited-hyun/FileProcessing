...
int BTree<keyType>::Remove (const keyType key, const int recAddr)
{
	...
	DN = FindLeaf(key); //인자로 받아온 key를 통해 삭제하려는 node를 저장해줌.
	...
	if (result != -1) {       //삭제려는 노드에 key의 최소개수 이상
		if (Large_flag) {     //key값이 가장 큰 값
			for (int i = 0; i < Height - 1; i++) {
	...
	else {	//node에 key가 최소개수 이하.
		BTreeNode<char>* TN = NULL;
		for (int i = 0; i < Nodes[Height - 2]->NumKeys; i++) {
			//삭제하려는 노드의 형제 중 합병 가능한 node가 존재하는지 check
			TN = Fetch(Nodes[Height - 2]->RecAddrs[i]);
			Large_key_merge = TN->LargestKey();
			if (TN->RecAddr == DN->RecAddr)
				continue;
			if (TN->Merge(DN))	// 합병 성공하면 더이상의 합병이 필요없으므로 loop 탈출.
				break;
		}
		PT = Nodes[Height - 2];  //부모노드 저장.

		if (TN->LargestKey() == DN->Keys[0]) {  //합병한 형제노드의 Largest key가 삭제노드의 가장 작은 키와 같을 때(왼쪽에 있는 노드와 합병되었을 때)
			int temp1, temp2;
			for (int i = 0; i < PT->NumKeys; i++) {
				if (PT->Keys[i] == Large_key)       //부모 노드의 키중 하나가 삭제될 노드의 Largest key인 경우.
					temp1 = i;
				if (PT->Keys[i] == Large_key_merge) //부모 노드의 키중 하나가 삭제노드와 합병된 형제노드의 Largest key인 경우.
					temp2 = i;
			}
			PT->RecAddrs[temp1] = PT->RecAddrs[temp2]; //부모노드의 Largest node 갱신.
			PT->Remove(Large_key_merge, recAddr); //부모노드의 Largest node 갱신되었으므로 이전의 것 삭제.
		}
		else     //왼쪽 노드와 합병된 것이 아닐 때
			PT->Remove(Large_key, recAddr);   //부모노드로부터 삭제될 노드에 대한 정보 삭제.
		Store(PT);
		Store(TN);
	}
	return -1;
